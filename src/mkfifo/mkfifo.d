...
int main(string[] args) {
	try {
		...
		return runApplication(args, (Program app) {
			...
			app.add(new Option("m", "mode",
				"Mode to create the fifo as. Default: 0666 - umask").defaultValue(""));
		}, (ProgramArgs args) {
			...
			// nasty ol libc style
			int result = mkfifo(args.arg("fifo").toStringz, target_mode).throwStdcErr();
			if (target_mode != base_mode) {
				// mkfifo() does not allow perms outside of the umask, so we fix it here.
				result = chmod(args.arg("fifo").toStringz, target_mode).throwStdcErr();
			}
			return result;
		});
	} catch (Exception ex) {
